...
public interface Property<T extends AccessibleObject> {
  ...
  static <T, U> U convert(T value, @NonNull Class<U> type) {
    ...
    val converter = (Function<T, U>) AbstractProperty.defaultConverters.get(
        Pair.of(value.getClass(), type));
   ...
   * myField; } } } then the <code> memberReadName()</code> is <code>getMyField()
