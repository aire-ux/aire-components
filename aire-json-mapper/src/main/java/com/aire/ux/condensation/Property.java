...
public interface Property<T extends AccessibleObject> {
  ...
  /**
   *
   * @return the <code>normalized</code> name of this member.  For instance, if
   * you have {@code
   * class Sample {
   *   int myField;
   *   int getMyField() {
   ...
   * }
   * }
   * then the <code>memberReadName()</code> is <code>getMyField()</code>
   * and the <code>memberWriteName()</code> is <code>setMyField()</code>
   * and the <code>memberNormalizedName</code> is <code>myField</code>.
   *
   * Note that the memberNormalizedName is not required to correspond to any field on the host
   * type
   */
