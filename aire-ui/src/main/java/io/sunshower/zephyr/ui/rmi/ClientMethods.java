...
public final class ClientMethods {
  ...
  public ClientMethods(
      @NonNull Supplier<UI> uiSupplier,
  ...
  public <T extends Serializable> ClientMethod<T> get(String name,
      Class<?>... argumentTypes) {
    return (ClientMethod<T>) internmap.computeIfAbsent(name,
        k -> new DefaultClientMethod<T>(writer, name,
