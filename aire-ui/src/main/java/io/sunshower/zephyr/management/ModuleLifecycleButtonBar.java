...
public class ModuleLifecycleButtonBar extends MenuBar {
  ...
  public ModuleLifecycleButtonBar(Grid<Module> grid, Zephyr zephyr, Module module) {
    ...
    startButton = createButton(VaadinIcon.PLAY_CIRCLE, "Start Module",
        ButtonVariant.LUMO_SUCCESS);
    stopButton = createButton(VaadinIcon.STOP, "Stop Module",
        ButtonVariant.LUMO_ERROR);
    restartButton = createButton(VaadinIcon.REFRESH, "Restart Module",
  ...
  private Registration configureRestartRegistration(Grid<Module> grid, Zephyr zephyr,
      Module module) {
    return restartButton.addClickListener(click -> {
      grid.select(module);
  ...
  private Registration configureStopRegistration(Grid<Module> grid, Zephyr zephyr,
      Module module) {
    return stopButton.addClickListener(click -> {
      grid.select(module);
      zephyr.stop(List.of(module.getCoordinate().toCanonicalForm()));
      fireEvent(new ModuleLifecycleChangedEvent(this, false));
  ...
  private Registration configureStartAction(Grid<Module> grid, Zephyr zephyr,
      Module module) {
    return startButton.addClickListener(click -> {
      grid.select(module);
      zephyr.start(module.getCoordinate().toCanonicalForm());
      fireEvent(new ModuleLifecycleChangedEvent(this, false));
  ...
  private Button createButton(VaadinIcon icon, String description,
      ButtonVariant variant) {
    val result = new Button(icon.create());
    result.addThemeVariants(ButtonVariant.LUMO_SMALL, ButtonVariant.LUMO_ICON,
        ButtonVariant.LUMO_PRIMARY, variant);
